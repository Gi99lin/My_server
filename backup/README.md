...
**Отказоустойчивость:** сбой одного дампа БД, retention (Step 3) или
integrity check (Step 4) логируется как warning и не прерывает остальной
бэкап — критичен только сам restic backup (Step 2). В конце ночного
прогона в Telegram уходит одно из:
- `✅ Backup completed successfully` — всё чисто
- `⚠️ ... with N warning(s)` — бэкап прошёл, но что-то по мелочи не дампнулось/не запрунилось — смотри лог
- `❌ Backup FAILED at line N` — сам restic backup не выполнился, бэкапа за эту ночь нет
- `⚠️ Weekly restic integrity check FAILED` — по воскресеньям, если `restic check` нашёл проблему в репозитории
